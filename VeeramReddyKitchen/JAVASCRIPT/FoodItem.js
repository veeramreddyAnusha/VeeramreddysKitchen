...
async function deleteProfile() {
    ...
    try {
        if (isAdmin) {
            const response = await fetch(`http://localhost:8080/deleteAdmin?adminId=${currentUser.adminId}`, { method: 'DELETE' });
            if (response.ok || response.status === 302) { alert('Profile deleted successfully!'); logout(); }
            else { alert('Failed to delete profile'); }
        } else {
            const response = await fetch(`http://localhost:8080/deleteUser?userId=${currentUser.userId}`, { method: 'DELETE' });
            if (response.ok || response.status === 302) { alert('Profile deleted successfully!'); logout(); }
            else { alert('Failed to delete profile'); }
        }
    } catch (error) {
            ...
            <div class="col-md-4 col-lg-3 mb-4">
                <div class="card food-item-card h-100 border-success" style="border: 2px dashed #198754;">
                    <div class="card-body d-flex flex-column justify-content-center align-items-center text-center">
                        ...
                        <button class="btn btn-success mt-3" onclick="showAddForm()">Add Item</button>
                    </div>
                </div>
            </div>
        `;
                ...
                <div class="card-body d-flex flex-column">
                    ...
                    ${isAdmin ? `
                        <div class="btn-group mt-2" role="group">
                            <button class="btn btn-warning btn-sm" onclick="editItem(${item.foodId})">Edit</button>
                            <button class="btn btn-danger btn-sm" onclick="deleteItem(${item.foodId})">Delete</button>
                        </div>
                    ` : `
...
function getImageSrc(item) {
    if (item.image) {
        if (typeof item.image === 'string') return `data:image/jpeg;base64,${item.image}`;
        if (item.image.length > 0) return `data:image/jpeg;base64,${btoa(String.fromCharCode(...item.image))}`;
    }
...
async function addToCart(foodId, quantity) {
    ...
    try {
        await fetch(`http://localhost:8080/save/${userData.userId}`, { method: 'POST' });
        const cartResponse = await fetch(`http://localhost:8080/fetch/${userData.userId}`);
        if (!cartResponse.ok) return Promise.reject('Failed to get cart');
        const cartData = await cartResponse.json();
        if (!cartData.data || !cartData.data.cartId) return Promise.reject('Invalid cart data');
        const cartId = cartData.data.cartId;
        await fetch(`http://localhost:8080/add?cartId=${cartId}&foodId=${foodId}&quantity=${quantity}`, { method: 'POST' });
...
async function removeFromCart(foodId) {
    ...
    try {
        ...
        if (cartData.data && cartData.data.cartItems) {
            ...
            if (cartItem) {
                await fetch(`http://localhost:8080/deleteCartItem/${cartItem.cartItemId}`, { method: 'DELETE' });
            }
        }
        return Promise.resolve();
    } catch (error) {
...
async function addNewItem() {
    ...
    if (!adminData || !adminData.adminId) { alert('Admin not logged in properly'); return; }
    ...
    try {
        const response = await fetch('http://localhost:8080/saveFoodItem', { method: 'POST', body: formData });
        if (response.ok) {
...
async function editItem(foodId) {
    ...
    const categoryOptions = categories.map(cat =>
        `<option value="${cat.categoryId}" ${cat.categoryId === (item.category?.categoryId || item.categoryId) ? "selected" : ""}>
            ${cat.categoryName}
        </option>`
    ).join("");
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label">Name</label>
                        <input id="editItemName" class="form-control" value="${item.name}" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Price (₹)</label>
                        <input id="editItemPrice" type="number" class="form-control" value="${item.price}" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Quantity</label>
                        <input id="editItemQty" class="form-control" value="${item.quantity}" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Category</label>
                        <select id="editItemCategory" class="form-select">${categoryOptions}</select>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Change Image</label>
                        <input type="file" id="editItemImage" class="form-control" accept="image/*">
                ...
                <div class="modal-footer">
                    ...
                    <button class="btn btn-success" onclick="updateItem(${foodId})">Update</button>
                </div>
            </div>
        </div>
    </div>`;
...
async function updateItem(foodId) {
    ...
    formData.append("quantity", document.getElementById("editItemQty").value.trim());
    formData.append("categoryId", document.getElementById("editItemCategory").value);
    ...
    try {
        const res = await fetch("http://localhost:8080/FoodItemUpdate", { method: "PUT", body: formData });
...
async function deleteItem(foodId) {
    ...
    try {
        const response = await fetch(`http://localhost:8080/deleteFoodItem?foodId=${foodId}`, { method: 'DELETE' });
        if (response.ok) { alert('Item deleted successfully!'); loadFoodItems(); }
...
function decrementCartCount() {
    ...
    if (count > 0) cartCount.textContent = count - 1;
}
...
async function updateCartCount() {
    ...
    try {
        ...
        if (cartData.data && cartData.data.cartItems && cartData.data.cartItems.length > 0) {
            cartCount.textContent = cartData.data.cartItems.reduce((sum, item) => sum + item.quantity, 0);
        } else {
...
function toggleCategoryAdminUI() {
    ...
    addCategorySection.style.display = isAdmin ? "flex" : "none";
}

async function loadItemsByCategory(categoryId) {
    try {
        const res = await fetch(`${BASE_URL}/FindFoodItemsByCategory?categoryId=${categoryId}`);
    ...
    fetch(`${BASE_URL}/FindAllCategoryies`)
        ...
        .then(res => {
            ...
            categoryMenu.innerHTML = `
                <li>
                    <span class="cat active" onclick="setActiveCategory(this); loadFoodItems()">All</span>
                </li>
            `;
            categories.forEach(c => {
                categoryMenu.innerHTML += `
                    <li data-id="${c.categoryId}">
                        <span class="cat" onclick="setActiveCategory(this); loadItemsByCategory(${c.categoryId})">${c.categoryName}</span>
                        ${isAdmin ? `
...
function renderFoodItems(items) {
    ...
    if (items.length === 0) { itemsContainer.innerHTML = "<p>No items found</p>"; return; }
    items.forEach(item => {
        itemsContainer.innerHTML += `
            <div class="food-card">
...
function addCategory() {
    ...
    }).then(() => { input.value = ""; fetchCategories(); });
}
...
function updateCategory(li, newName, oldName) {
    ...
    if (!newName || newName === oldName) { fetchCategories(); return; }
    fetch(CATEGORY_UPDATE_URL, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ categoryId: Number(li.dataset.id), categoryName: newName })
    })
...
function deleteCategory(id) {
    ...
    fetch(`${BASE_URL}/deleteCategory?categoryId=${id}`, { method: "DELETE" })
    .then(async res => {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    fetchCategories();
    loadFoodItems();
    if (!isAdmin && currentUser) updateCartCount();
});
